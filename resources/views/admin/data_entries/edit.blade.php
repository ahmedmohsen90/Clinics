...
@section('content')
    <div class="col-md-12 col-xl-12">
        <div class="card">
            ...
            <form action="{{ aurl('data_entries/update/' . $dentry->id) }}" enctype="multipart/form-data" method="POST">
                @csrf
                <div class="card-body">
                    <div class="mb-3">
                        <label class="floating-label" for="name">{{ trans('admin.Name') }} <span
                                class="redStar">*</span></label>
                        <input type="text" name="name" value="{{ $dentry->name }}" class="form-control" id="name">
                    </div>
                    <div class="mb-3">
                        <label class="floating-label" for="mobile">{{ trans('admin.Mobile') }} <span
                                class="redStar">*</span></label>
                        <input type="number" name="mobile" value="{{ $dentry->mobile }}" class="form-control" id="mobile">
                    </div>
                    <div class="mb-3">
